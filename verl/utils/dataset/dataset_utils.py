    ...
    def collate_variable_batch(self, batch: list[dict[str, any]]) -> dict[str, any]:
        ...
        for key in tensor_keys:
            if isinstance(batch[0][key], torch.Tensor):
                ...
                if tensors[0].dim() >= 2:
                    # VLM position_ids need explicit jagged-dim handling when
                    # all samples share the same seq_len; other tensors fall
                    # back to PyTorch's standard construction.
                    final_batch[key] = tu.reconstruct_nested_tensor(tensors, key=key)
                else:
