...
@ToolParser.register("answer")
class AnswerToolParser(ToolParser):
    """
    Tool parser that extracts tool name from <answer>tool_name</answer> tags.
    
    This parser is designed for image restoration tasks where the model
    outputs the tool/action name directly in an answer tag, e.g.:
        <answer>ridcp</answer>
        <answer>scunet</answer>
        <answer>stop</answer>
    
    The tool name is used as both the function name and as the 'action' argument.
    """

    def __init__(self, tokenizer) -> None:
        super().__init__(tokenizer)
        self.answer_start_token: str = "<answer>"
        self.answer_end_token: str = "</answer>"
        # Match <answer>tool_name</answer>, capturing the tool name
        self.answer_regex = regex.compile(r"<answer>\s*([\w_]+)\s*</answer>", regex.IGNORECASE)
    ...
    async def extract_tool_calls(self, responses_ids: list[int]) -> tuple[str, list[FunctionCall]]:
        ...
        if self.answer_start_token not in text.lower() or self.answer_end_token not in text.lower():
            return text, []

        matches = self.answer_regex.findall(text)
        function_calls = []
        for match in matches:
            tool_name = match.strip().lower()
            if tool_name:
                # For restoration, use "restore_image" as the function name
                # and pass the actual tool as the "action" argument
                function_calls.append(
                    FunctionCall(
                        name="restore_image",  # The registered tool name in config
                        arguments=json.dumps({"action": tool_name}, ensure_ascii=False)
                    )
                )
                logger.info(f"Extracted tool call from <answer>: {tool_name}")

        # Remaining text excluding answer tokens
        content = self.answer_regex.sub("", text)
